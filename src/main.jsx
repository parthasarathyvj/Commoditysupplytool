...
function FreightManagement({ timeframe, onTimeframeChange }) {
  const [records, setRecords] = useState([
    { plant: 'BELOIT PLANT', growingArea: 'North Valley', planValue: '1,240', actualValue: '1,180' },
    { plant: 'CASA GRANDE PLANT', growingArea: 'Prairie South', planValue: '980', actualValue: '1,025' },
    { plant: 'CAMBRIDGE PLANT', growingArea: 'Maple Ridge', planValue: '760', actualValue: '720' },
  ])
  ...
  const filteredRecords = records.filter((record) => Object.values(record).join(' ').toLowerCase().includes(search.toLowerCase()))
  return <section className="card"><div className="card-header"><h3>Freight Management</h3><span className="badge ok">Plan vs Actual</span></div><div className="filters-row"><select><option>All Commodities</option><option>Wheat</option><option>Oats</option><option>Oil</option><option>Corn</option></select><select><option>All Regions</option><option>East - US</option><option>West - US</option><option>Central - US</option><option>Canada</option></select><select><option>All Currencies</option><option>USD</option><option>CAD</option></select><TimeframeFilter value={timeframe} onChange={onTimeframeChange} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Plant or growing area" /></div><div className="table-wrap"><table><thead><tr><th>Plant</th><th>Growing Area</th><th>Plan Value</th><th>Actual Value</th><th>Action</th></tr></thead><tbody>{filteredRecords.map((record) => <tr key={`${record.plant}-${record.growingArea}`}><td>{record.plant}</td><td>{record.growingArea}</td><td>{record.planValue}</td><td>{record.actualValue}</td><td><button className="btn small" type="button">Edit</button></td></tr>)}</tbody></table></div><form className="form-grid" onSubmit={addRecord}><label>Plant<input value={form.plant} onChange={(event) => updateField('plant', event.target.value)} placeholder="Plant name" /></label><label>Growing Area<input value={form.growingArea} onChange={(event) => updateField('growingArea', event.target.value)} placeholder="Growing area" /></label><label>Plan Value<input type="number" value={form.planValue} onChange={(event) => updateField('planValue', event.target.value)} placeholder="0" /></label><label>Actual Value<input type="number" value={form.actualValue} onChange={(event) => updateField('actualValue', event.target.value)} placeholder="0" /></label><div className="modal-actions full-field"><button className="btn primary" type="submit">Add Freight Record</button></div></form></section>
}
