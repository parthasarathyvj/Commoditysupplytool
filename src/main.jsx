...
function AdminWorkspace({ query }) {
  ...
  return <div className="admin-sections">{adminSections.map((section) => {
    ...
    return <section className="card admin-card" key={section.title}><div className="card-header"><h3>{section.title}</h3><button className="btn primary small">+ Add</button></div><div className="table-wrap"><table><thead><tr>{section.headers.map((head) => <th key={head}>{head}</th>)}<th>Action</th></tr></thead><tbody>{data.map((row) => <tr key={row.join('-')}>{row.map((cell, index) => <td key={cell}>{index === row.length - 1 ? <span className={`badge ${['Active'].includes(cell) ? 'ok' : ['Inactive'].includes(cell) ? 'off' : 'warn'}`}>{cell}</span> : cell}</td>)}<td><button className="btn small">Edit</button></td></tr>)}</tbody></table></div></section>
  })}</div>
}
...
function EditPopup({ record, onClose, onSave }) {
  const [notes, setNotes] = useState(record.notes || '')
  return <div className="modal-backdrop" onClick={onClose}><div className="modal" onClick={(event) => event.stopPropagation()}><div className="card-header"><h3>Edit {record.name}</h3><button className="close" onClick={onClose}>×</button></div><div className="form-grid"><label>Record<input defaultValue={record.name} /></label><label>Category<input defaultValue={record.category} /></label><label>Status<select defaultValue={record.status}><option>Active</option><option>Approved</option><option>Review</option><option>Pending</option><option>Inactive</option><option>Exception</option></select></label><label>Effective From<input type="date" /></label><label className="full-field">Notes<textarea rows="5" value={notes} onChange={(event) => setNotes(event.target.value)} /></label></div><div className="modal-actions"><button className="btn" onClick={onClose}>Cancel</button><button className="btn primary" onClick={() => { onSave(); onClose() }}>Save Changes</button></div></div></div>
}
...
function Dashboard({ tab, setTab, onLogout }) {
  ...
  const volumeData = { labels: ['East - US', 'Central - US', 'West - US', 'Canada'], datasets: [{ label: 'Volume Actual', data: [8420, 7210, 6830, 5960], backgroundColor: ['#004c97', '#3b82f6', '#f4b000', '#138a4b'] }] }
  ...
  const handleEditClick = (event) => { const button = event.target.closest('button'); if (!button || button.textContent.trim() !== 'Edit') return; const row = button.closest('tr'); setEditRecord({ name: row?.cells?.[0]?.textContent || 'Record', category: row?.cells?.[1]?.textContent || tab, status: 'Active', notes: row?.textContent || '' }) }
  return <div className="app-shell" onClick={handleEditClick}>
    ...
    <div className="sub-nav"><div><button className="active">US</button><button>US-Core</button><button>Co-Man</button><button>Canada</button><button onClick={() => setSubView('details')}>Details</button><button onClick={() => setSubView('exceptions')}>Exceptions</button></div><div className="toolbar"><button className="outline" onClick={() => setModal(true)}>PC Volume Plan</button><button className="outline" onClick={() => setFormModal('feedback')}>Feedback</button><button className="outline" onClick={() => setFormModal('alert')}>Alerts</button><select value={period} onChange={(event) => { setPeriod(event.target.value); setMessage(`${event.target.value} data refreshed`) }}><option>Period</option><option>Week</option><option>Month</option></select></div></div>
    <main className="page"><div className="page-head"><div><p className="eyebrow">CST / OPERATIONS</p><h2>{tab === 'Home' ? 'Home Dashboard' : tab}</h2><p>Commodity supply overview for <b>{period}</b> with current performance and recent trends.</p></div><div className="actions"><button className="btn" onClick={() => setMessage(`${period} data refreshed`)}>Refresh</button><button className="btn primary" onClick={() => setMessage(`${period} export generated`)}>Export</button></div></div>
    ...
    {editRecord && <EditPopup record={editRecord} onClose={() => setEditRecord(null)} onSave={() => setMessage(`${editRecord.name} updated successfully`)} />}
    ...
    {copilot && <Copilot initialMessage={message} activeTab={tab} period={period} onClose={() => { setCopilot(false); setMessage('') }} />}
  </div>
}
