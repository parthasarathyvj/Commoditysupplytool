...
function FreightManagement({ timeframe, onTimeframeChange }) {
  const [records, setRecords] = useState([
    { plant: 'BELOIT PLANT', growingArea: 'North Valley', year: '2024-2025', period: 'P1', week: 'W1', planValue: '1,240', actualValue: '1,180' },
    { plant: 'CASA GRANDE PLANT', growingArea: 'Prairie South', year: '2025-2026', period: 'P2', week: 'W2', planValue: '980', actualValue: '1,025' },
    { plant: 'CAMBRIDGE PLANT', growingArea: 'Maple Ridge', year: '2023-2024', period: 'P3', week: 'W3', planValue: '760', actualValue: '720' },
  ])
  const [search, setSearch] = useState('')
  const [form, setForm] = useState({ plant: '', growingArea: '', planValue: '', actualValue: '' })
  const updateField = (field, value) => setForm((current) => ({ ...current, [field]: value }))
  const addRecord = (event) => {
    event.preventDefault()
    if (!form.plant || !form.growingArea || !form.planValue || !form.actualValue) return
    setRecords((current) => [...current, { ...form, year: timeframe }])
    setForm({ plant: '', growingArea: '', planValue: '', actualValue: '' })
  }
  const filteredRecords = records.filter((record) => record.year === timeframe && Object.values(record).join(' ').toLowerCase().includes(search.toLowerCase()))
  return <section className="card"><div className="card-header"><h3>Freight Management</h3><span className="badge ok">Plan vs Actual</span></div><div className="filters-row"><select><option>All Commodities</option><option>Wheat</option><option>Oats</option><option>Oil</option><option>Corn</option></select><select><option>All Regions</option><option>East - US</option><option>West - US</option><option>Central - US</option><option>Canada</option></select><select><option>All Currencies</option><option>USD</option><option>CAD</option></select><TimeframeFilter value={timeframe} onChange={onTimeframeChange} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Plant or growing area" /></div><div className="table-wrap"><table><thead><tr><th>Plant</th><th>Growing Area</th><th>Period</th><th>Week</th><th>Plan Value</th><th>Actual Value</th><th>Action</th></tr></thead><tbody>{filteredRecords.map((record) => <tr key={`${record.plant}-${record.growingArea}-${record.week}`}><td>{record.plant}</td><td>{record.growingArea}</td><td>{record.period}</td><td>{record.week}</td><td>{record.planValue}</td><td>{record.actualValue}</td><td><button className="btn small" type="button">Edit</button></td></tr>)}</tbody></table></div><form className="form-grid" onSubmit={addRecord}><label>Plant<input value={form.plant} onChange={(event) => updateField('plant', event.target.value)} placeholder="Plant name" /></label><label>Growing Area<input value={form.growingArea} onChange={(event) => updateField('growingArea', event.target.value)} placeholder="Growing area" /></label><label>Period<select value={form.period || 'P1'} onChange={(event) => updateField('period', event.target.value)}><option>P1</option><option>P2</option><option>P3</option><option>P4</option></select></label><label>Week<select value={form.week || 'W1'} onChange={(event) => updateField('week', event.target.value)}><option>W1</option><option>W2</option><option>W3</option><option>W4</option></select></label><label>Plan Value<input type="number" value={form.planValue} onChange={(event) => updateField('planValue', event.target.value)} placeholder="0" /></label><label>Actual Value<input type="number" value={form.actualValue} onChange={(event) => updateField('actualValue', event.target.value)} placeholder="0" /></label><div className="modal-actions full-field"><button className="btn primary" type="submit">Add Freight Record</button></div></form></section>
}
...
function Dashboard({ tab, setTab, onLogout }) {
  ...
  return <div className="app-shell" onClick={handleEditClick}>
    ...
    <main key={`${tab}-${region}-${period}`} className="page"><div className="page-head"><div><p className="eyebrow">CST / OPERATIONS</p><h2>{tab === 'Home' ? 'Home Dashboard' : tab}</h2><p>Commodity supply overview for <b>{region}</b> and <b>{period}</b> with current performance and recent trends. Last refreshed: {refreshedAt}</p></div><div className="actions"><button className="btn" onClick={refreshDashboard}>Refresh</button><button className="btn primary" onClick={() => { exportDashboardCsv(tab, period, region); setMessage(`${region} ${period} export downloaded`) }}>Export</button></div></div>
      ...
      {tab === 'Home' && <div className="stat-grid"><div className="stat-box"><span>Daily ERP refresh</span><strong>In Progress</strong><small>ERP data synchronization is running</small></div></div>}
